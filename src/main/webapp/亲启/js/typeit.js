 ...
 (function($){
   ...
   _proto.deleteLoop = function() {
    ...
    this.deleteTimeout = setTimeout(function () {
      ...
      $(this.theElement).find('.ti-text-container').text(shortenedText);
       ...
       if (this.deleteCount < this.phraseLength && (typeof (this.settings.stopDeleteWord)== 'undefined' || shortenedText.substring(shortenedText.length-this.settings.stopDeleteWord.length)!=this.settings.stopDeleteWord)) {
         this.deleteLoop();
  ...
  // stop the plugin from typing or deleting stuff whenever it's called
   _proto.stopTyping = function() {
      ...
      $(this.theElement).find('.ti-text-container').removeClass('ti-cursor');//停止打字时光标消失
   };
}(jQuery));
